# T-aiorg_pm_bot-012 통합 보고서

## 원 요청
최근 2026년 3월 기준 코딩에이전트(오픈소스 포함) 시장 조사해주고 기술적으로 트렌드를 파악해줘

## 최종 전달본
2026년 3월 기준 코딩 에이전트 시장과 기술 트렌드를 조사했습니다. 핵심 결론은 하나입니다: **코딩 에이전트는 이미 "자동완성 툴" 단계를 졸업하고, 수시간 단위 작업을 스스로 완료하는 "자율 소프트웨어 엔지니어" 단계에 진입했습니다.** SWE-bench 80% 돌파와 MCP 프로토콜 표준화가 그 전환점입니다.

---

**📊 시장 현황 — 규모 $74억**

| 제품 | 점유율·사용자 | 밸류에이션 | 월 가격 |
|------|------------|----------|--------|
| GitHub Copilot | 42%, 누적 2,000만 유저 | Microsoft 산하 | $10~$39 |
| Cursor | 18%, ARR $1B+ | $29.3B (→$50B 협상 중) | $20 |
| Windsurf | LogRocket 랭킹 1위 | Cognition 인수 ~$250M | $15 |
| Devin | ARR ~$150M | Cognition $10.2B | $20+종량제 |
| Claude Code | 개발자 선호 46% 1위 | Anthropic $61.5B | $20~$100 |

**오픈소스 Top 4**
- **OpenHands** ★68,884 — E2E 자율 에이전트, 오픈소스 최대
- **Aider** — GPT-5 연동 시 SWE-bench 88% (상용 추월 구간 발생)
- **Cline** — VS Code 500만+ 설치, 오픈소스 채택 1위
- **SWE-agent** ★18,705 — Princeton 기반, 학술 연구 특화

경쟁 구도: Cursor가 Copilot 1위를 직접 위협. OpenAI의 Windsurf 인수($3B)·Cognition의 Devin+Windsurf 통합으로 IDE+자율에이전트 M&A가 가속화되고 있습니다.

---

**⚡ 5대 기술 트렌드**

**1. SWE-bench 80% 장벽 돌파** (2년 만에 9배 성장)
Claude Opus 4.5 80.9%, Gemini 3.1 Pro 80.6%, GPT-5.2 80.0% — 상위 5개 모두 80%+. 단, 장기·복합 과제용 SWE-bench Pro에서는 여전히 23% 상한으로 실전과의 괴리는 존재합니다. HumanEval은 99%로 포화 → LiveCodeBench·SWE-bench Pro가 신뢰 지표로 부상 중.

**2. RAG → 에이전틱 검색으로 전환**
Claude Code는 벡터 DB를 버리고 "도구로 코드 탐색" 방식을 채택했습니다. tree-sitter 기반 semantic chunking이 업계 코드 인덱싱 표준으로 정착했습니다.

**3. 프로토콜 표준화 완성**
MCP 월 9,700만 다운로드 + A2A = 전 업계 표준 (Linux Foundation 거버넌스로 이관). Llama 4 Scout은 context window 10M 토큰까지 확장됐으나 실질 성능 괴리는 주의가 필요합니다.

**4. "100줄의 역설"**
Princeton mini-SWE-agent는 100줄 Python으로 74%를 달성했습니다. 복잡한 스캐폴드보다 강한 모델이 핵심. 멀티에이전트는 Planner → Worker → Judge 3-role 패턴이 표준화됐습니다.

**5. 오픈소스의 추격**
Qwen 2.5 Coder HumanEval 88.4%, DeepSeek V3.2가 GPT-5급 성능을 저비용으로 제공. 엔터프라이즈의 사내 자체 호스팅 수요가 급증하고 있습니다.

---

**📅 전망**
- **6개월**: MCP vs 독자 프로토콜 표준 전쟁. SWE-bench Pro가 주력 평가 지표로 교체
- **1년**: 소형 특화 코드 모델이 범용 LLM을 코딩 태스크에서 대체. 보안 샌드박스가 제품 필수 스펙화

---

상세 데이터는 첨부 파일 **coding_agent_market_2026_03.md**, **20260316_coding_agent_tech_trends.md** 에서 확인하실 수 있습니다.

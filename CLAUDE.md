...
- **대상 모델 예시**: `gemini-2.5-flash-preview-image-generation` 등 Preview 계열
...
#### 각 조직(봇) 결과 보고 시 의무 기재 규칙
PM을 제외한 모든 specialist 조직(개발실/운영실/디자인실/기획실/성장실/리서치실)은 작업 완료 보고 시 아래를 반드시 준수한다:

1. **기재 위치**: 결과물(산출물) 섹션 바로 아래 또는 보고서 맨 마지막
2. **기재 형식**: `**사용 에이전트/페르소나**: [에이전트명1, 에이전트명2]`
3. **에이전트명 기준**: `~/.claude/agents/` 에 실제 존재하는 파일명 (확장자 제외)
4. **기재 생략 금지**: 에이전트 없이 단독 처리한 경우 `[해당 없음]` 으로 명시

**절대 금지 패턴** (PM 최종 보고에 아래 형태 포함 불가):
```
❌ 투입 페르소나: [개발실]
❌ 사용 에이전트: engineering-senior-developer  ← PM 보고서에 작성
❌ 투입 인원: [에이전트명]  ← PM 최종 취합 보고에 포함
```
...
# 2. main에 머지
git checkout main
git merge fix/<작업명> --no-ff

# 3. push (운영실만)
git push origin main

# 4. 재시작 요청 (watchdog가 restart_bots.sh → start_all.sh 체인 실행)
bash scripts/request_restart.sh --reason "변경 사유"
```

**절대 금지 패턴**:
```
❌ worktree 안에서 바로 commit만 하고 request_restart.sh 호출
   → start_all.sh가 main HEAD로 worktree 재생성 → 커밋 유실

❌ restart_bots.sh 직접 호출
   → watchdog 우회 위험

❌ watchdog 미실행 상태에서 request_restart.sh 호출
   → 플래그만 남고 재시작 안 됨 (watchdog 생존 여부 먼저 확인)
```

**재시작 체인 전체 흐름**:
```
request_restart.sh → ~/.ai-org/restart_requested 플래그
    → watchdog 감지 → restart_bots.sh
        → 봇 종료 → start_all.sh
            → .worktrees/bot-runtime 재생성 (main HEAD 기준)
            → 봇 재시작
```
...
#### 프로젝트 소스코드 수정 후 커밋 의무 (전체 조직 공통)

봇이 프로젝트 레포의 **소스코드·설정·문서를 수정**했으면 반드시 커밋한다. 커밋 없이 방치하면 다른 봇 작업과 충돌하고, 변경 추적이 불가능해진다.

**규칙**:
1. 프로젝트 파일(`.py`, `.yaml`, `.md`, `.sh`, `.ts`, `.js`, `.html` 등)을 수정했으면 **작업 완료 시점에 즉시 커밋**
2. 커밋 메시지는 `conventional commits` 형식: `feat:`, `fix:`, `docs:`, `chore:`, `refactor:` 등
3. **논리적 단위로 분리**: 관련 없는 변경을 한 커밋에 섞지 않는다
4. `.env`, `*.db`, `*.log` 등 gitignore 대상은 커밋하지 않는다
5. 커밋 후 `git push origin main` (또는 현재 브랜치)으로 리모트에 반영

**금지**:
- 파일 수정 후 커밋 없이 세션 종료
- 10개 이상 파일을 하나의 커밋으로 몰아넣기
- 커밋 메시지 없이 또는 "update" 같은 무의미한 메시지로 커밋
...
- **🔒 시크릿/토큰 보안 (필수)**:
  - 봇 토큰, API 키, 비밀번호 등 민감 정보를 코드에 절대 하드코딩 금지.
  - 반드시 `.env` 환경변수 또는 CI secrets로 관리.
  - 테스트 코드에서도 예외 없음 — `os.environ.get("VAR_NAME")` 사용.
  - `.env`, `.env.*` 파일은 `.gitignore`에 포함되어 있으므로 커밋되지 않음.
  - git 커밋 히스토리에 토큰이 남지 않도록 주의. 실수로 커밋 시 즉시 토큰 재발행.

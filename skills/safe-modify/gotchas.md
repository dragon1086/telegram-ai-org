...
## Gotcha 8: 작업 도중 커밋/머지를 미루다 작업 소실

**상황**: 워크트리에서 긴 코드 수정 세션을 진행하면서 "완성되면 한 번에 커밋하자"며 중간 커밋 없이 작업.

**증상**: 세션 중단(컨텍스트 창 소진, 프로세스 종료, 충돌 등)으로 워크트리 변경사항 전체 소실. 또는 머지 시 충돌 해소가 복잡해져 이전 작업과 혼재.

**해결**:
- **코드 수정 작업 중 의미 있는 단위로 자주 커밋** (기능 1개 완성, 버그 1개 수정 등)
- 커밋 후 **가능하면 즉시 main 브랜치에 머지** — 워크트리 격리 기간을 최소화
- 산출물 생성(문서, 보고서, 데이터) 작업은 예외 — 완성 후 1회 커밋 가능
- 규칙: "세션 종료 전 반드시 커밋하고 main에 머지"

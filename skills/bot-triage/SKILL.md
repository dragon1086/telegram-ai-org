...
> **Step 3c(프로세스 강제 종료), Step 3d(전체 재시작/git push)는 infra 역할 조직만 직접 실행.**
> infra 역할 조직 = `organizations.yaml`에서 `capabilities`에 `infra`가 포함된 조직 (org ID는 조직 구성마다 다름).
> infra 역할 조직이 아닌 모든 specialist 조직은 이 단계에서 직접 실행하지 말고, COLLAB으로 위임한다:
> ```
...
> 🔒 **infra 역할 조직 전용 실행 단계.** 타 부서는 3번까지 진단하고 infra 역할 조직에 COLLAB 요청.

1. `.env` 파일 검증 — 필수 토큰/키 존재 확인
2. DB 연결 테스트 — `python -c "import aiosqlite"`
3. 네트워크 확인 — `curl -s https://api.telegram.org`
4. 전체 재시작 — `bash scripts/request_restart.sh --reason "글로벌 장애 복구"` (infra 역할 조직만 실행)

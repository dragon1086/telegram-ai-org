...
| **`git push`** | **infra 역할 조직 위임 필수** ⛔ | 개발실 자체 수행 금지 (PM 명시 지시 예외) |
| **`git merge`** | **infra 역할 조직 위임 필수** ⛔ | 개발실 자체 수행 금지 (PM 명시 지시 예외) |
| **봇 재기동** | **infra 역할 조직 위임 필수** ⛔ | 개발실 자체 수행 금지 (PM 명시 지시 예외) |

> infra 역할 조직: `organizations.yaml`에서 `capabilities: [infra]`가 설정된 조직.
> 조직 구성에 따라 다를 수 있으므로 org ID를 하드코딩하지 말 것.

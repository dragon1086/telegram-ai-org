...
```bash
# 1. 저장소 포크 후 클론
git clone https://github.com/your-username/telegram-ai-org.git
cd telegram-ai-org
...
1. PR 생성 → GitHub Actions CI 자동 실행 (lint + test)
...
├── test_context_window.py           # 컨텍스트 창 단위 테스트 (16개)
├── test_pm_context_injection.py     # PM 컨텍스트 주입 통합 테스트 (7개)
├── e2e/
│   ├── test_engine_compat_e2e.py    # 3엔진 호환성 E2E
│   └── test_pm_dispatch_e2e.py      # PM 라우팅 E2E
└── ...
```
...
1. [기존 이슈](https://github.com/your-org/telegram-ai-org/issues) 검색 — 중복 확인

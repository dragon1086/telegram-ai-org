...
class TelegramRelay:
    ...
    async def on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if MessageClassifier.is_bot_sender(sender):
            # Phase 1c 리팩토링 — 봇 메시지 처리를 분리된 모듈에 위임
            if ENABLE_REFACTORED_DISPATCHER and ENABLE_REFACTORED_PM_HANDLER:
                # 협업 요청 → bot_dispatcher, PM_TASK/토론/완료 → pm_message_handler
                await _dispatch_bot_message(self, text, update, context)
                await _pm_handle_bot_message(self, text, update, context)
            else:
                # 레거시 인라인 처리 (ENABLE_REFACTORED_DISPATCHER=0 또는 ENABLE_REFACTORED_PM_HANDLER=0 시)
    ...
    # ── Phase 1c: 명령어 핸들러 — relay_command_handlers 위임 ─────────────────

    def _ensure_runtime_bootstrap(self) -> bool:
        """세션 부트스트랩 — relay_command_handlers 위임."""
        return _cmd._ensure_runtime_bootstrap(self)

    async def on_self_added_to_chat(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """봇 그룹 추가 자동 초기화 — relay_command_handlers 위임."""
        await _cmd.on_self_added_to_chat(self, update, context)

    async def on_command_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/start — relay_command_handlers 위임."""
        await _cmd.on_command_start(self, update, context)

    async def on_command_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/status — relay_command_handlers 위임."""
        await _cmd.on_command_status(self, update, context)

    async def on_command_reset(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/reset — relay_command_handlers 위임."""
        await _cmd.on_command_reset(self, update, context)

    async def on_command_schedule(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/schedule — relay_command_handlers 위임."""
        await _cmd.on_command_schedule(self, update, context)

    async def on_command_schedules(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/schedules — relay_command_handlers 위임."""
        await _cmd.on_command_schedules(self, update, context)

    async def on_command_cancel_schedule(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/cancel_schedule — relay_command_handlers 위임."""
        await _cmd.on_command_cancel_schedule(self, update, context)

    async def on_command_pause_schedule(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/pause_schedule — relay_command_handlers 위임."""
        await _cmd.on_command_pause_schedule(self, update, context)

    async def on_command_resume_schedule(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/resume_schedule — relay_command_handlers 위임."""
        await _cmd.on_command_resume_schedule(self, update, context)

    async def on_command_stop_tasks(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/stop_tasks — relay_command_handlers 위임."""
        await _cmd.on_command_stop_tasks(self, update, context)

    async def on_command_restart(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/restart — relay_command_handlers 위임."""
        await _cmd.on_command_restart(self, update, context)

    async def on_command_engine(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """/engine — relay_command_handlers 위임."""
        await _cmd.on_command_engine(self, update, context)
    ...
    async def _setup_callback_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """설정 마법사 메뉴 — relay_command_handlers 위임."""
        return await _cmd._setup_callback_menu(self, update, context)

    async def _setup_receive_token(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """설정 마법사 토큰 수신 — relay_command_handlers 위임."""
        return await _cmd._setup_receive_token(self, update, context)

    async def _setup_receive_engine(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """설정 마법사 엔진 선택 — relay_command_handlers 위임."""
        return await _cmd._setup_receive_engine(self, update, context)

    async def _setup_receive_identity(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """설정 마법사 정체성 수신 — relay_command_handlers 위임."""
        return await _cmd._setup_receive_identity(self, update, context)

    async def _setup_cancel(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """설정 마법사 취소 — relay_command_handlers 위임."""
        return await _cmd._setup_cancel(self, update, context)
            ...
            try:
                ...
                for parent_id in candidates:
                    ...
                    for sibling in siblings:
                        # FIX(T-783): sibling은 DB dict 또는 SubTask dataclass 모두 가능
                        # dict.get()은 dataclass에 없으므로 getattr() fallback으로 타입 안전하게 처리
                        _sib_status = sibling.get("status") if isinstance(sibling, dict) else getattr(sibling, "status", None)
                        _sib_meta = (sibling.get("metadata") if isinstance(sibling, dict) else getattr(sibling, "metadata", None)) or {}
                        if (
                            _sib_status == "done"
                            and _sib_meta.get("collab")
                        ):
...
# ── 유틸 ──────────────────────────────────────────────────────────────────
    ...
    async def _execute_pm_task(self, task_info: dict) -> None:
        ...
        TASK_EXEC_TOTAL_TIMEOUT_SEC = int(os.environ.get("TASK_EXEC_TOTAL_TIMEOUT_SEC", "3600"))  # 절대 상한 60분 (기존 30분은 복잡한 다단계 태스크에 너무 짧음)
        ...
        try:
            ...
            # CancelledError 원인 추적: "hang" 또는 "total_timeout"
            _cancel_reason: list[str] = ["hang"]
            ...
            async def _hang_watchdog() -> None:
                ...
                while True:
                    ...
                    if elapsed_total > TASK_EXEC_TOTAL_TIMEOUT_SEC:
                        ...
                        _cancel_reason[0] = "total_timeout"
                        if _exec_task and not _exec_task.done():
                            _exec_task.cancel()
                        return
            ...
            try:
                ...
                reason = _cancel_reason[0]
                # 부분 결과 보존 시도 (runner가 _last_partial_output에 저장했을 수 있음)
                _partial_result = ""
                try:
                    _partial_result = getattr(self._runner, "get_last_partial_output", lambda: "")()
                    if _partial_result:
                        logger.info(
                            f"[{self.org_id}] 취소 전 부분 결과 보존 ({len(_partial_result)}자): "
                            f"{_partial_result[:200]}"
                        )
                        # 부분 결과를 runbook에 기록
                        self._append_runbook(
                            run_id,
                            f"Partial result (cancelled: {reason})",
                            _partial_result[:3000],
                            phase_name="implementation",
                        )
                        # DB에도 부분 결과 저장 (재시도 시 참고 가능)
                        await self.context_db.update_pm_task_status(
                            task_id, "running",
                            result=f"[부분결과-취소전] {_partial_result[:500]}"
                        )
                except Exception as _pr_err:
                    logger.debug(f"[{self.org_id}] 부분 결과 보존 실패 (무시): {_pr_err}")
                if reason == "total_timeout":
                    elapsed = time.monotonic() - _task_start_time
                    logger.error(
                        f"[{self.org_id}] 태스크 {task_id} TOTAL TIMEOUT으로 강제 취소됨 "
                        f"(총 {elapsed:.0f}s 경과, 한도={TASK_EXEC_TOTAL_TIMEOUT_SEC}s). "
                        f"TaskPoller가 재시도합니다."
                    )
                    raise RuntimeError(
                        f"{engine_label} task {task_id} exceeded total time limit "
                        f"(total timeout {TASK_EXEC_TOTAL_TIMEOUT_SEC}s)"
                    )
                else:
                    elapsed_idle = time.monotonic() - _last_activity[0]
                    logger.error(
                        f"[{self.org_id}] 태스크 {task_id} HANG DETECT로 강제 취소됨 "
                        f"({elapsed_idle:.0f}s 무활동, 임계값={HANG_DETECT_TIMEOUT_SEC}s). "
                        f"TaskPoller가 재시도합니다."
                    )
                    raise RuntimeError(
                        f"{engine_label} process hang detected for task {task_id} "
                        f"(no activity for {elapsed_idle:.0f}s, threshold={HANG_DETECT_TIMEOUT_SEC}s)"
                    )
            finally:
            ...
            # PM 태스크 완료 후 COLLAB 태그를 실제로 라우팅한다 (자율 협업 활성화).
            # 부서 봇이 [COLLAB:...] 태그를 응답에 포함하면 크로스팀 협업으로 즉시 발동.
            _bot_for_collab = self.app.bot if self.app else None
            response = await self._handle_collab_tags(
                response or "",
                bot=_bot_for_collab,
            ...
            # 투입 페르소나 정보 추가 — 각 조직 완료보고 결론 하단에 포함
            try:
                _persona_footer = self._team_builder.format_persona_footer(team_config)
                if _persona_footer:
                    public_result = public_result + _persona_footer
            except Exception as _pf_err:
                logger.debug("완료보고 투입 페르소나 footer 생성 실패 (무시): {}", _pf_err)
...
# ── Phase 1c: 봇 설정 유틸리티 — relay_bot_setup.py로 이전됨 ──────────────────
# 아래 함수들은 core/relay_bot_setup.py로 추출되었습니다.
# telegram_relay.py 상단 import 블록에서 relay_bot_setup으로부터 재-import함.

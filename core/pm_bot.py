...
class PMBot:
    """PM Bot: 유저 요청 → 태스크 분해 → Worker 봇 할당."""
    ...
    async def handle_user_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # 봇 메시지 무시 (구조화된 OrgMessage 헤더 포함 시)
        ...
        # 태스크 생성
        # TODO: Claude API로 태스크 분해 + R&R 할당 (현재는 단순 dev_bot 할당)
        worker_bots = ["@dev_bot"]
        task = await self.task_manager.create_task(
            description=user_text,
            assigned_to=worker_bots,
        )
        ...
        logger.info(f"태스크 {task.id} 할당 완료 → {worker_bots}")
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        if update.message:
            await update.message.reply_text(
                "🤖 PM Bot 온라인. AI 조직 준비 완료.\n"
                "요청사항을 입력하면 팀원들에게 태스크를 할당합니다."
            )

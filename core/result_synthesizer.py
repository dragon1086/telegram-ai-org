...
_SYNTHESIS_PROMPT = (
    ...
    "REPORT:\n"
    "[structured final report — see format rules below]\n"
    "END_REPORT\n\n"
    "Rules:\n"
    ...
    "- Write in Korean\n"
    "## REPORT format rules (strictly follow this structure):\n"
    "  The REPORT section must follow this EXACT structure — no exceptions:\n\n"
    "  ## 결론\n"
    "  [한 문장으로 핵심 결론 — 무엇이 어떻게 됐는지, 사용자의 원래 요청에 직접 답]\n\n"
    "  ## 핵심 내용\n"
    "  [PM 관점에서 부서 결과를 통합·정리. 가장 중요한 사실만. 표/목록 활용 권장.]\n\n"
    "  ## 판단 근거 (필요시만)\n"
    "  [왜 이 결론인지. 충돌·이슈가 있었으면 여기에만 명시. 없으면 이 섹션 생략.]\n\n"
    "  ## 다음 조치 (있을 때만)\n"
    "  [후속 태스크가 있으면 명시. 없으면 이 섹션 전체 생략.]\n\n"
    "## REPORT content rules:\n"
    "- DO NOT copy department outputs verbatim. Synthesize, condense, and reframe from PM perspective.\n"
    "- DO NOT repeat the same finding across multiple sections.\n"
    "- DO NOT include internal operational notes (COLLAB tags, '역할은 여기서 끝', '운영실에 위임' etc.).\n"
    "- Each key fact appears ONCE only — if multiple departments say the same thing, merge into one line.\n"
    "- The REPORT must be readable as a standalone document without knowing department internals.\n"

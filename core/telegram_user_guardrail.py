...
async def ensure_user_friendly_output(
    ...
    *,
    ...
    if decision_client is not None and should_rewrite:
        ...
        prompt = (
            "You are a PM writing a final report for a Telegram-only end user.\n"
            "Rewrite the draft into a clean, structured report in Korean.\n\n"
            "## Required report structure (follow exactly):\n"
            "  ## 결론\n"
            "  [한 문장 핵심 결론 — 사용자 요청에 직접 답. 첫 번째 섹션 필수.]\n\n"
            "  ## 핵심 내용\n"
            "  [중요한 사실만 정리. 표/목록 활용. 부서별 원문 복사 금지.]\n\n"
            "  ## 다음 조치 (있을 때만)\n"
            "  [후속 태스크가 있으면 명시. 없으면 이 섹션 전체 생략.]\n\n"
            "## Rules:\n"
            "- First section (결론) must directly answer the user's original request in ONE sentence.\n"
            "- DO NOT copy raw department outputs verbatim — synthesize and condense.\n"
            "- Each key fact appears ONCE only — no repetition across sections.\n"
            "- Remove internal operational notes: COLLAB tags, '역할은 여기서 끝', '운영실에 위임', "
            "  '[TEAM:...]', '[ARTIFACT:...]', or any bot-internal control language.\n"
            "- Do not expose local filesystem paths — refer to files by filename only.\n"
            "- Preserve all factual claims, numbers, commit hashes, and proper nouns.\n"
            "- If artifacts exist, refer to them by filename only at the very end.\n"
